...
public class J{
    ...
    static class Task{
        ...
        List<Map<Integer,Integer>> player;
        int n, k;
        Pair wildcard;
        ...
        public void solve(InputReader in, PrintWriter out) {
            ...
            this.n= in.nextInt(); this.k= in.nextInt();
            this.wildcard = new Pair(k-1, false);
            this.player= new ArrayList<>();
            for(int i=0;i<n;i++) player.add(new HashMap<>());
            for(int i=0;i<n;i++){
                char[] temp= in.nextStringCharArr();
                Map<Integer,Integer> cnt= player.get(i);
                for(char ch: temp){
                    int val= -1;
                    if(ch=='A') val=1;
                    else if(ch>='2' && ch<='9') val=ch-'0';
                    else if(ch=='D') val=10;
                    else if(ch=='Q') val=11;
                    else if(ch=='J') val=12;
                    else if(ch=='K') val=13;
            ...
            int turn =k-1;
            int winner=-1;
            while((winner=terminate())==-1){

                if(wildcard.turn==turn){
                    if(wildcard.already){
                        wildcard.turn= (wildcard.turn+1)%n;
                        wildcard.already=false;
                        turn= (turn+1)%n;
                        continue;
                    }
                    else{
                        wildcard.already=true;
                    }
                }

                Map<Integer,Integer> cnt= player.get(turn);
                ...
                for(Map.Entry<Integer,Integer> entry: cnt.entrySet()){
                    if(entry.getValue()==minFreq){
                        chosen= Math.min(chosen, entry.getKey());
                    }
                }
                // reduce 1 for number chosen for player turn
                cnt.put(chosen, cnt.get(chosen)-1);
                if(cnt.get(chosen)==0) cnt.remove(chosen);

                // increase 1 for number chosen for player turn +1
                Map<Integer,Integer> next= player.get((turn+1)%n);
                next.put(chosen, next.getOrDefault(chosen, 0)+1);
        ...
        private int terminate(){
            int winner=-1, minKey=69;
            for(int i=0;i<n;i++){
                if(wildcard.turn==i) continue;
                Map<Integer,Integer> cnt= player.get(i);
                for(Map.Entry<Integer,Integer> entry: cnt.entrySet()){
                    if(entry.getValue()<1) cnt.remove(entry.getKey()); 
                }
